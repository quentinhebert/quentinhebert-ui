...
export default function AdminQandAForm(props) {
  /********** PROPS **********/
  const { handleClose } = props

  const { setSnackSeverity, setSnackMessage } = useContext(AppContext)

  const { mutate } = useSWR("QandA")
  ...
  const handleSuccess = () => {
    ...
    setSnackMessage("FAQ mise à jour")
    handleClose()
  ...
  const handleError = () => {
    ...
    setSnackMessage("FAQ non mise à jour")
  }
  const handleSave = async () => {
    const res = await apiCall.application.QandA.update(richTextValue)
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ width: "100%" }}
      >
        <ModalTitle>Modifier la FAQ</ModalTitle>
      </motion.div>
